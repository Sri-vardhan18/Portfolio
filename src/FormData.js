...
const data = [
  {
    image: image,
    URLType: 'https://64b547a4529e9900086d34d8--frolicking-pothos-0c5f40.netlify.app/',
    Title:'Foodify'
  },
  {
    image: image2,
...
const resumeDriveLink = 'https://drive.google.com/uc?id=1dx1ITtLmyAofgk_QxrrySccYIMmNgTMZ';
      ...
      {data.map((item, index) => (
        <div key={index} className='card'>
          ...
          <div className='belowsize'>
            <h5 >{item.Title}</h5>
            <button onClick={() => openWindow(item.URLType)} className='demo-button'>Demo</button>
          </div>
        </div>
      ))}
      <div className='card'>
        ...
        <h5>Resume</h5>
        <button onClick={downloadResume} className='demo-button'>Download</button>
      </div>
